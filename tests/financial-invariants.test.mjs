...
const isProduction = url?.includes('qpczmbvqflaqwvyphepf') ?? false;
const configured = Boolean(url && key && businessId && invoiceId && customerId && accountId && gatewayId);

if (configured && isProduction) {
  throw new Error('Financial integration tests refuse to run against production.');
}

const db = configured ? createClient(url, key) : null;
const requireDb = () => {
  assert.ok(db, 'Financial test fixtures are not configured.');
  return db;
};
...
test('product/service invoice fixture is supplied', { skip: !configured }, async () => {
  ...
  const { data, error } = await client.from('invoice_items')
    .select('id,item_type,product_service_id')
    .eq('invoice_id', invoiceId);
  assert.ifError(error);
  assert.ok((data ?? []).length > 0, 'Invoice fixture must contain at least one item');
  const types = new Set((data ?? []).map(x => x.item_type));
  assert.ok([...types].some(x => x === 'product' || x === 'service'),
    'Fixture must use canonical product/service item types');
});
...
test('invoice totals are internally consistent', { skip: !configured }, async () => {
  ...
  assert.equal(
    Number(data.total),
    Number(data.subtotal) - Number(data.discount_total) + Number(data.tax_total)
  );
});
