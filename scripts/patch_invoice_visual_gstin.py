...
# Make the business query include the actual registration state used by the
# business-creation flow. Without this, the renderer cannot reliably distinguish
# a tax-registered business from a non-tax-registered business.
...
# Tax registration helper. Explicit false/unregistered modes always suppress the
# identifier; otherwise a real tax identifier is required before it is printed.
...
# Put the geometric color accents in the shared template stylesheet so they apply
# consistently to every invoice template and survive the print iframe. Receipts
