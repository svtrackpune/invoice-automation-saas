-- Keep draft invoice GST eligibility aligned with the authoritative business tax profile.
-- This replaces the legacy businesses.tax_settings flag used by the draft workflow.
...
begin
  ...
  select coalesce(
    tp.tax_regime = 'GST' and coalesce(tp.gst_registration_type,'NONE') <> 'COMPOSITION',
    false
  ) into v_tax_enabled
  from public.business_tax_profiles tp
  where tp.business_id=inv.business_id;
