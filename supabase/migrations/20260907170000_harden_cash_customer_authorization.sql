-- Security hardening: Cash Customer is a business-scoped system customer.
-- Preserve authenticated access for the Cash Bill workflow, but require the
-- caller to have customer-management permission for the target business.
...
begin
  ...
  where business_id=p_business_id
    and display_name='Cash Customer'
    and is_active=true
  order by created_at
  limit 1;
  ...
  if v_id is null then
    insert into public.customers(
      ...
      notify_customer,notify_owner,product_reminder_after_unit,
      service_reminder_after_unit
    )
    values(
      p_business_id,'Cash Customer','Cash Customer',null,null,null,'N/A',
      '{}'::jsonb,'{}'::jsonb,0,0,
      'System walk-in customer used by Cash Bill.',
      '{"system":"cash_bill"}'::jsonb,true,false,0,'none',0,'both',0,
