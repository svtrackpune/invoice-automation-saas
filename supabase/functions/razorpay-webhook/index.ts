...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!providerLinkId) {
      ...
      }).eq("event_id", eventId).eq("provider", "razorpay");
      return json({ ok: true, ignored: true });
    }

    const { data: link } = await admin
      .from("payment_links")
      .select("id,business_id,invoice_id,status,provider_link_id")
    ...
    if (!link) {
      await admin.from("payment_webhook_events").update({
        status: "ignored",
        processed_at: new Date().toISOString(),
        error_message: "Payment link not registered",
      }).eq("event_id", eventId).eq("provider", "razorpay");
      return json({ ok: true, ignored: true });
    }
    ...
    if (eventType === "payment_link.paid" || eventType === "payment_link.partially_paid") {
      ...
      const { error: paymentError } = await admin.rpc("record_gateway_payment", {
        p_provider: "razorpay",
        p_provider_link_id: providerLinkId,
        p_provider_transaction_id: payment?.id || `${providerLinkId}:${eventId}`,
        p_event_id: eventId,
        p_amount: amount,
        p_payment_date: new Date().toISOString().slice(0, 10),
        p_notes: `Razorpay ${eventType}`,
      });
