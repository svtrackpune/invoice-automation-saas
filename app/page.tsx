...
function Setup({onDone}:{onDone:(businessId:string)=>void}){const[name,setName]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState('');
async function create(){setBusy(true);setError('');const r=await supabase.rpc('create_business_for_current_user',{p_business_name:name,p_business_type:'sole_proprietorship',p_country_code:'IN',p_currency_code:'INR',p_tax_enabled:true,p_tax_region:'IN-MH'});if(r.error){setError(r.error.message);setBusy(false);return}const contexts=await supabase.rpc('get_my_business_context');const rows=(contexts.data||[]) as Array<{business_id:string;business_name:string}>;const returned=typeof r.data==='string'?r.data:Array.isArray(r.data)?(typeof r.data[0]==='string'?r.data[0]:r.data[0]?.business_id):r.data?.business_id;const created=rows.find(x=>x.business_id===returned)||rows.filter(x=>x.business_name===name.trim()).at(-1)||rows.at(-1);setBusy(false);if(created){localStorage.setItem('moneymatters.activeBusinessId',created.business_id);onDone(created.business_id);return}setError('Business was created, but its workspace could not be opened. Please refresh and try again.');}
return <main className="grid min-h-screen place-items-center bg-[#fbfaff] p-5"><div className="w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-8 shadow-xl sm:p-10"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-600 font-black text-white">M</div><p className="mt-6 text-xs font-bold uppercase tracking-[.18em] text-violet-600">First-time setup</p><h1 className="mt-2 text-3xl font-semibold">Create your business</h1><p className="mt-2 text-sm leading-6 text-slate-500">Start with the business name. After creation, we’ll open a profile window for the complete address, contacts and customer-facing details used across Moneymatters.</p><div className="mt-7"><Field label="Business name"><Input value={name} onChange={e=>setName(e.target.value)} placeholder="Your business name" autoFocus/></Field></div>{error&&<div className="mt-4 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</div>}<div className="mt-7 flex justify-end"><Button disabled={busy||!name.trim()} onClick={create}>{busy?'Creating business…':'Create business →'}</Button></div></div></main>}

export default function Page(){const[session,setSession]=useState<any>(null),[hasBusiness,setHasBusiness]=useState<boolean|null>(null),[loading,setLoading]=useState(true);const check=async(s:any)=>{setSession(s);if(!s){setHasBusiness(null);setLoading(false);return}const r=await supabase.rpc('get_my_business_context');setHasBusiness(!!r.data?.length);setLoading(false)};useEffect(()=>{supabase.auth.getSession().then(({data})=>check(data.session));const{data:{subscription}}=supabase.auth.onAuthStateChange((_e,s)=>check(s));return()=>subscription.unsubscribe()},[]);useEffect(()=>{if(session&&hasBusiness){window.location.replace('/next-workspace')}},[session,hasBusiness]);if(loading)return <div className="grid min-h-screen place-items-center bg-[#fbfaff] text-sm text-slate-500">Loading Moneymatters…</div>;if(!session)return <Auth onReady={()=>supabase.auth.getSession().then(({data})=>check(data.session))}/>;if(!hasBusiness)return <Setup onDone={(businessId)=>{localStorage.setItem('moneymatters.activeBusinessId',businessId);window.location.replace(`/next-workspace/business-details?business=${encodeURIComponent(businessId)}`)}}/>;return <div className="grid min-h-screen place-items-center bg-[#fbfaff] text-sm text-slate-500">Opening your workspace…</div>}
