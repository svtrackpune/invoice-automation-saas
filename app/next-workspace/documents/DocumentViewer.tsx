...
function Tagline({ business, fields, compact = false }: { business: any; fields: any; compact?: boolean }) {
  const tagline = text(business.tagline || fields.tagline || '');
  return <div className={`tagline${compact ? ' tagline-compact' : ''}`} aria-label="Business tagline">
    {tagline || <span className="tagline-placeholder">Your tagline</span>}
  </div>;
}
...
function BusinessIdentity({ business, logoUrl, fields }: { business: any; logoUrl: string; fields: any }) {
  return <div className="business-identity">
    <Logo url={logoUrl} />
    <Tagline business={business} fields={fields} />
    <div>
...
function Paper({ type, payload, business, customer, items, theme, fields, total, balance, received, logoUrl }: any) {
  ...
  if (receipt) return <article className={`paper receipt-paper ${theme.dark ? 'theme-dark' : ''}`} style={{ '--accent': theme.accent, '--table': theme.table, '--line': theme.line } as React.CSSProperties}>
    <div className="receipt-head">
      <Logo url={logoUrl} />
      <Tagline business={business} fields={fields} compact />
  ...
  return <article className={`paper ${theme.dark ? 'theme-dark' : ''}`} style={{ '--accent': theme.accent, '--table': theme.table, '--line': theme.line } as React.CSSProperties}>
    <header className="document-header">
      <BusinessIdentity business={business} logoUrl={logoUrl} fields={fields} />
