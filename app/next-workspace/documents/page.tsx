...
function InvoiceDocument({p,b,c,items,st,templateKey,type,cf,total,balance,logoUrl}:{p:any;b:any;c:any;items:any[];st:TemplateMeta;templateKey:string;type:string;cf:any;total:number;balance:number;logoUrl:string}){
  ...
  return <article className={`invoice-paper template-${templateKey}`} style={{'--accent':st.accent,'--table':st.table,'--soft':st.soft,'--line':st.line} as React.CSSProperties}>
    ...
    <div className="platform-footer"><a href="https://mm.nilanga.in" target="_blank" rel="noreferrer">Moneymatters</a></div>
  </article>;
}
...
function DocumentsContent(){
  ...
  const id=qs.get('id')||'';
  const[job,setJob]=useState<Job|null>(null),[template,setTemplate]=useState<Template|null>(null),[docPref,setDocPref]=useState<any>(null),[business,setBusiness]=useState<any>(null),[logoUrl,setLogoUrl]=useState(''),[loading,setLoading]=useState(true),[error,setError]=useState(''),[notice,setNotice]=useState('');
  ...
  const canEditInvoice=type==='invoice'&&p.status==='draft';
  const editUrl=canEditInvoice?`/next-workspace/invoices/new?edit=${id}`:'';

  const printDocument=()=>{
    const paper=document.querySelector('.invoice-paper') as HTMLElement|null;
    if(!paper)return;
    ...
    const styles=Array.from(document.head.querySelectorAll('style')).map(x=>x.textContent||'').join('\n');
    printDoc.open();
    printDoc.write(`<!doctype html><html><head><meta charset="utf-8"><title>Document</title><style>${styles}\nhtml,body{margin:0!important;padding:0!important;background:#fff!important}body>*{display:block!important}.document-page{padding:0!important;background:#fff!important;min-height:0!important}.document-toolbar{display:none!important}.invoice-paper{width:794px!important;min-height:1123px!important;margin:0!important;box-shadow:none!important;border:0!important}.platform-footer{display:block!important;text-align:center!important}</style></head><body>${paper.outerHTML}</body></html>`);
    printDoc.close();
    ...
    if(images.length===0){window.setTimeout(print,80);return;}
    let remaining=images.length;
    const done=()=>{remaining-=1;if(remaining<=0)window.setTimeout(print,80)};
  ...
  const copyLink=async()=>{
    ...
    try{await navigator.clipboard.writeText(documentUrl);setNotice('Invoice link copied to clipboard.');window.setTimeout(()=>setNotice(''),2500)}catch{setNotice('Unable to copy the link.');}
  };
  const shareDocument=async()=>{
    if(!documentUrl)return;
    try{
      if(navigator.share){await navigator.share({title:`${title} ${number}`,text:`${b.name||'Business'} · ${number} · ${money(balance,b.currency_code)} due`,url:documentUrl});}
      else{await navigator.clipboard.writeText(documentUrl);setNotice('Invoice link copied to clipboard.');window.setTimeout(()=>setNotice(''),2500)}
    }catch{/* User cancelled native share. */}
  };
  const back=()=>{if(editUrl){location.href=editUrl;return}history.back()};

  return <main className="document-page">
    <div className="document-toolbar print:hidden"><div><div className="toolbar-kicker">Document center</div><h1>{title} {number}</h1><p>{templates[templateKey]?.name||template?.template_name||'Professional'} · Template v{job?.template_version||1} · Print-ready</p></div><div className="toolbar-actions"><button onClick={back} className="toolbar-secondary">{editUrl?'Back to edit':'Back'}</button>{type==='invoice'&&<><button onClick={()=>{void shareDocument()}} className="toolbar-secondary">Share</button><button onClick={()=>{void copyLink()}} className="toolbar-secondary">Copy link</button></>}<button onClick={printDocument} className="toolbar-primary">Print / Save PDF</button></div></div>
    {notice&&<div className="document-notice print:hidden">{notice}</div>}
    ...
    <style jsx global>{`
      *{box-sizing:border-box}.document-page{min-height:100vh;background:#eef1f6;padding:28px 18px;color:#0f172a}.document-toolbar{max-width:794px;margin:0 auto 18px;display:flex;justify-content:space-between;align-items:center;gap:20px}.toolbar-kicker{font-size:10px;font-weight:800;letter-spacing:.18em;text-transform:uppercase;color:#7c3aed}.document-toolbar h1{margin:3px 0 0;font-size:20px;font-weight:700}.document-toolbar p{margin:4px 0 0;font-size:12px;color:#64748b}.toolbar-actions{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}.toolbar-primary,.toolbar-secondary{border-radius:10px;padding:10px 14px;font-size:13px;font-weight:700;cursor:pointer}.toolbar-primary{border:0;background:#0f172a;color:white}.toolbar-secondary{border:1px solid #cbd5e1;background:white;color:#334155}.document-notice{max-width:794px;margin:0 auto 12px;border:1px solid #ddd6fe;background:#faf8ff;color:#5b21b6;border-radius:10px;padding:9px 12px;font-size:12px;font-weight:700}
