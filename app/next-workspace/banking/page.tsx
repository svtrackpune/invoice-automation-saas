...
export default function Banking(){
 ...
 const categorize=async(txn:Tx,accountId:string,matchType?:string)=>{if(!currentRec||currentRec.status==='locked'||!accountId)return;setBusy(true);setNotice('');const r=await supabase.rpc('match_bank_transaction',{p_reconciliation_id:currentRec.id,p_bank_transaction_id:txn.id,p_match_type:matchType|| (String(txn.direction).toLowerCase().includes('out')?'expense':'manual'),p_matched_record_id:accountId,p_notes:matchType==='transfer'?'Internal bank transfer':'Categorized in Banking'});if(r.error)setNotice(r.error.message);else{setNotice(matchType==='transfer'?'Internal transfer recorded without income/expense impact.':'Transaction categorized and posted to the ledger.');await load()}setBusy(false)};
