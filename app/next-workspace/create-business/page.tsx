...
import { applyIndustryPreset, type BusinessFeatureConfig } from '@/lib/business-config';
...
export default function CreateBusiness() {
  ...
  const selectedCategory = categories.find((category) => category.id === categoryId)?.name || null;
  const selectedSubcategory = subcategories.find((subcategory) => subcategory.id === subcategoryId)?.name || null;
  ...
  const next = () => {
    ...
    if (step === 2 && sellingModel === 'services') setInventoryEnabled(false);
  ...
  const create = async () => {
    ...
    if (created) {
      localStorage.setItem('moneymatters.activeBusinessId', created.business_id);
      if (inventoryEnabled && sellingModel !== 'services') {
        const inventory = await supabase.rpc('enable_business_inventory', { p_business_id: created.business_id });
